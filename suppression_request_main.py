...
class Suppression_Request:
    def __init__(self):
        ...
        self.filter_and_match_file_sources_consumer_kill_condition = False
        self.match_or_filter_file_sources_loaded = []
        self.match_sources_loaded = []
        self.filter_sources_loaded = []
    ...
    def filter_and_match_file_sources_producer(self, type_of_request, file_source_queue, file_source_details, match_or_filter_queue_empty_condition, thread_count, main_logger):
    ...
    def filter_and_match_file_sources_consumer(self, type_of_request, file_source_queue, match_or_filter_queue_empty_condition, main_logger, main_request_details):
        try:
            ...
            main_logger.info(f"Consumer execution started: {time.ctime()}")
            while True:
                if not self.filter_and_match_file_sources_consumer_kill_condition:
                    with match_or_filter_queue_empty_condition:
                    ...
                    self.match_or_filter_file_sources_loaded.append(load_match_or_filter_file_sources(type_of_request, file_source, file_source_index, main_request_details))
    ...
    def perform_match_or_filter_selection(self, type_of_request,filter_details, main_request_details, main_request_table ,pid_file, mysql_cursor, main_logger, current_count):
        if type_of_request == "SUPPRESS_MATCH":
        ...
        if len(match_or_filter_source_details) != 0:
            ...
            producer_thread = threading.Thread(target=self.filter_and_match_file_sources_producer, args=(type_of_request, match_or_filter_file_source_queue, match_or_filter_file_source_details, match_or_filter_queue_empty_condition, THREAD_COUNT, main_logger))
            producer_thread.start()
            consumer_threads = []
            for i in range(THREAD_COUNT):
                # consumer_logger = create_logger(f"consumer_logger_{i}", log_to_stdout=True)
                consumer_thread = threading.Thread(target=self.filter_and_match_file_sources_consumer, args=(
    ...
    # Main function
    def suppression_request_processor(self, supp_request_id, run_number, schedule_time=None, notification_mails=""):
        try:
            ...
            if len(self.sources_loaded) != self.input_sources_count:
                ...
                mysql_cursor.execute(UPDATE_SCHEDULE_STATUS, ('E', '0', f'Only {len(self.sources_loaded)} sources are successfully processed out of {self.input_sources_count} sources.', supp_request_id, run_number))
                update_next_schedule_due("SUPPRESSION_REQUEST", supp_request_id, run_number, main_logger)
                os.remove(pid_file)
                return
...
if __name__ == "__main__":
    try:
        ...
        supp_obj.suppression_request_processor(supp_request_id, run_number, schedule_time, notification_mails)
