...
$mysql_path -u$app_db_user -p$app_db_pass -h$app_db_host -D$app_db_name -A -vv -e "UPDATE $SUPPRESSION_REQUEST_OFFERS_TABLE SET STATUS='I' where requestId=$request_id and requestScheduledId=$schedule_id and runNumber=$run_number and offerId=$offer_id "
...
if [[ "$offer_download_status" != "Success"* ]]; then
    ...
    $mysql_path -u$app_db_user -p$app_db_pass -h$app_db_host -D$app_db_name -A -ss -e "UPDATE $SUPPRESSION_REQUEST_OFFERS_TABLE SET STATUS='E' where requestId=$request_id and requestScheduledId=$schedule_id and runNumber=$run_number and offerId=$offer_id "
