...
def load_data_sources_producer(sources_queue, request_id, queue_empty_condition, count, main_logger):
    ...
    with queue_empty_condition:
        for _ in range(count):  # Put sentinel value for each consumer
...
def load_data_sources_consumer(sources_queue, main_datasource_details, queue_empty_condition, consumer_logger):
    try:
        ...
        while True:
            ...
            if source is None:  # Sentinel value indicating end of tasks
                ...
                break
            sources_loaded += load_data_source(source, main_datasource_details, consumer_logger)
...
def main(request_id):
    ...
    producer_thread = threading.Thread(target=load_data_sources_producer,
                                       args=(sources_queue, request_id, queue_empty_condition, count, main_logger))
    ...
    create_main_datasource(sources_loaded, main_datasource_details, request_id)
